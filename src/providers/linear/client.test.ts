...
describe("Linear connection client", () => {
  it("uses an OAuth callback URL and records the installed workspace identity", async () => {
      ...
      fetch: async (url, init) => {
        ...
        if (requestUrl.endsWith("/oauth/token")) {
          return json({
            ...
            expires_in: 3600,
            scope: "read,comments:create,app:mentionable,app:assignable,write",
          });
        }
        return json({
    ...
    assert.equal(
      ...
      "read,comments:create,app:mentionable,app:assignable,write",
    );
  ...
  it("requires write access for native Agent Activity updates", () => {
    assert.equal(
